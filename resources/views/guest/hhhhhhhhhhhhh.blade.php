...
{{--
    <div>
        ...
        <p>Quantity: {{ session('qte') }}</p>
    </div>
--}}

{{-- @foreach (session('cart') as $key => $item)
<div>
    <h4>produit :{{ $item['product_id'] }}</h4>
                  ...
                  @endif
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between mb-3 pt-1">
                            ...
                            <!--   une méthode getTotal() dans le modèle  commande   -->
...
<script>
    // Code JavaScript pour afficher le modal et gérer la confirmation du paiement
    document.getElementById('paymentModalButton').addEventListener('click', function() {
        $('#paymentModal').modal('show');
    });

    // Gérer l'affichage des champs de paiement en ligne en fonction du mode de paiement sélectionné
    ...
    // Collecter les données du formulaire de confirmation
    var paymentMethod = document.getElementById('paymentMethod').value;
    var phoneNumber = document.getElementById('phoneNumber');
    var deliveryAddress = document.getElementById('deliveryAddress');
    var email = document.getElementById('email');

    // Mettre à jour le formulaire de récapitulatif de panier avec les données de confirmation
    ...
    // Ajouter des champs cachés pour les données de contact et de livraison
    paymentForm.innerHTML += '<input type="hidden" name="phoneNumber" value="' + phoneNumber.value + '">';
    paymentForm.innerHTML += '<input type="hidden" name="deliveryAddress" value="' + deliveryAddress.value + '">';
    paymentForm.innerHTML += '<input type="hidden" name="email" value="' + email.value + '">';

    // Remettre à zéro les styles d'erreur Bootstrap et de validation
    phoneNumber.classList.remove('is-invalid', 'is-valid');
    deliveryAddress.classList.remove('is-invalid', 'is-valid');
